...
from operators import TokenNotFoundError
from subroutines import ancestors
from subroutines import jit_check_constraint_violation, \
...
from utils import import_custom_source
...
class Constraint(Prior):
    ...
    def make_constraint(self, mask, tokens):
        ...
        for t in tokens:
            prior[mask, int(t)] = self.mask_val
        return prior
    ...
    def is_violated(self, actions, parent, sibling):
        ...
        warnings.warn("{} ({}) {} : Using a slower version of constraint for Deap. You should write your own.".format(caller.filename, caller.lineno,
                                                                                                                      type(self).__name__))
...
class ConstConstraint(RelationalConstraint):
    ...
    def __init__(self, library):
        targets = library.const_token  # 如果operator_list没有常数c, 这里就为None
...
class RepeatConstraint(Constraint):
    ...
    def __init__(self, library, tokens, min_=None, max_=None):
        ...
        self.min = min_
        self.max = max_
        self.n_objects = 1  # Program.n_objects
...
class LengthConstraint(Constraint):
    ...
    def initial_prior(self):
        ...
        for t in self.library.terminal_tokens:
            # print('t',t)
            prior[int(t)] = -np.inf  # 初始的时候把终端节点的值设为负无穷
        return prior
